def parse_worksheet(lines):
    ...
    problems = []
    current_problem = {'numbers': [], 'operations': []}
    in_problem = False
    ...
    while col < len(numbers_line):
        ...
        if char == ' ' and op_char == ' ':
            ...
            if op_char in ['+', '*', '|']:
                current_problem['operations'].append(op_char)
...
def solve_problem(numbers, operations):
    """
    Solve a single problem by applying the operations to all numbers.
    Operations can be +, *, or | (concatenation).
    """
    if not numbers:
        return 0
    ...
    for i, num in enumerate(numbers[1:]):
        if i < len(operations):
            ...
            if op == '+':
                ...
                # Concatenation: convert to strings, concatenate, then convert back
...
for problem in problems:
    answer = solve_problem(problem['numbers'], problem['operations'])
    grand_total += answer
    print(f"Problem: {problem['numbers']} {problem['operations']} = {answer}")
